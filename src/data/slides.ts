/**
 * Slide HTML template functions — locale-aware
 */
import { getContent, brand as BR, type Locale } from './content';
...
export function getSlideHTML(locale: Locale = 'zh'): string[] {
  ...
  return [
    ...
    (() => {
      ...
      return `<section class="slide s-cover is-active" data-index="0" data-title="${C.slideTitles[0]}"><div class="red-bar"></div><div class="content"><p class="c-label anim-in">${d.label}</p><h1 class="anim-in-2">Enter<span class="dot">.</span>pro</h1><p class="subtitle anim-in-3">${d.subtitle}</p><p class="slogan anim-in-4">"${BR.slogan}"</p><div class="meta anim-in-5">${meta}</div></div></section>`;
    })(),

    // Slide 02 — What
    (() => {
      const d = C.what;
      return `<section class="slide s-what" data-index="1" data-title="${C.slideTitles[1]}"><div class="s-inner"><div class="s-col-left left anim-in"><p class="section-label">${d.chapter}</p><h2>${d.title}</h2><p class="slogan" style="margin-top:16px">"Just press Enter,<br>ship like a pro."</p></div><div class="s-col-right right anim-in-2"><p class="section-label">${d.introLabel}</p><p class="intro-text">${d.intro}</p><div style="width:32px;height:2px;background:var(--red);margin-bottom:24px;"></div><div class="links">${renderLinks(d.linkKeys)}</div></div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-audience bg-white" data-index="2" data-title="${C.slideTitles[2]}"><div class="header-row"></div><div class="inner"><div class="left-title anim-in"><p class="section-label">${d.chapter}</p><h2>${d.title}</h2><p style="margin-top:14px;font-size:10px;color:var(--gray-500);line-height:1.7;">${d.tagline}</p></div><div class="items">${items}</div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-quickstart" data-index="3" data-title="${C.slideTitles[3]}"><div class="inner"><div class="header anim-in"><p class="section-label" style="margin:0">${d.chapter}</p><h2>${d.title}</h2></div><div class="steps">${steps}</div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-plan bg-white" data-index="5" data-title="${C.slideTitles[5]}"><div class="top-accent"></div><div class="inner"><div class="left anim-in"><p class="section-label">${d.chapter}</p><span class="badge">${d.badge}</span><h2>${d.title}</h2><p>${d.intro}</p></div><div class="right">${feats}<div class="divider"></div><div class="tip anim-in-5">${d.tip}</div></div></div></section>`;
    })(),
    ...
    (() => {
      ...
      const mc = d.mcp;
      const skillItems = sk.items.map((item, i) => `<div class="skill-item${i === 0 ? ' featured' : ''}"><h5>${item.icon} ${item.title}</h5><p>${item.body}</p></div>`).join('');
    ...
    (() => {
      ...
      return `<section class="slide s-integrations" data-index="9" data-title="${C.slideTitles[9]}"><div class="inner"><div class="header anim-in"><p class="section-label" style="margin:0">${d.chapter}</p><h2>${d.title}</h2></div><div class="cards"><div class="cards-col-left">${leftCard}</div><div class="cards-col-right">${rightCards}</div></div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-templates bg-white" data-index="10" data-title="${C.slideTitles[10]}"><div class="s-inner"><div class="s-col-left left anim-in" style="border-right:1px solid var(--gray-100);"><p class="section-label">${d.chapter}</p><h2>${d.title}</h2><p style="font-size:10px;color:var(--gray-500);line-height:1.65;margin:12px 0 14px;">${d.templateDesc}</p><div class="tpl-tags">${tplTags}</div></div><div class="s-col-right right anim-in-2"><p class="section-label">${d.modelsLabel}</p><h3 style="font-size:16px;font-weight:900;color:var(--black);margin-bottom:18px;">${d.modelsTitle}</h3><div>${modelRows}</div></div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-credits" data-index="11" data-title="${C.slideTitles[11]}"><div class="s-inner"><div class="s-col-left left anim-in"><p class="section-label" style="color:rgba(255,255,255,0.35)">${d.chapter}</p><h2>${d.title}</h2>${freeItems}</div><div class="s-col-right right anim-in-2"><h3>${d.sidebarHint}</h3><div class="balance"><p class="hint">${d.balance.hint}</p><p class="amount">${d.balance.amount}</p><p class="unit">${d.balance.unit}</p></div><div class="plan-box"><span style="font-size:18px;">${d.plan.icon}</span><div><h4>${d.plan.title}</h4><p>${d.plan.body}</p></div></div></div></div></section>`;
    })(),
    ...
    (() => {
      ...
      return `<section class="slide s-contact bg-white" data-index="13" data-title="${C.slideTitles[13]}"><div class="red-border-left"></div><div class="s-inner"><div class="s-col-left left anim-in" style="border-right:1px solid var(--gray-100);"><p class="section-label">${d.sectionLabel}</p><h2>${d.title}</h2><p class="tagline">${d.tagline}</p><div class="cta" onclick="window.open('${d.ctaUrl}','_blank')"><span>${d.ctaText}</span><span class="arr">\u2192</span></div></div><div class="s-col-right right anim-in-2"><h3>${d.contactsTitle}</h3>${contacts}<div class="wechat-note">${d.wechatNote}</div></div></div></section>`;
    })(),
  ];
}
