...
export default function App() {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    setExporting(true);
    try {
      await exportSlidesToPdf(stageRef.current);
    } finally {
      // Restore the current slide after export
      const stage = stageRef.current;
      if (stage) {
        const slides = stage.querySelectorAll('.slide');
        slides.forEach(s => s.classList.remove('is-active'));
  ...
  return (
    <div id="app">
      ...
      <div id="controls">
        ...
        <button className={`ctrl-btn${exporting ? ' is-exporting' : ''}`} title={exporting ? C.ui.exporting : C.ui.exportPdf} onClick={handleExportPdf} disabled={exporting}>
          {exporting ? (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="spin-icon"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
          ) : (
